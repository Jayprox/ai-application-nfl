...
*Updated 2026-09-18.* Part 2 (agent team) is fully live as of this update — all five roles (calibration/tracking, ranking, edge, portfolio, conversational/chat), a Player Props board with real-time odds and final-game grading, and all three vendor integrations (nflverse, Highlightly, The Odds API) are built and running in production. §5's natural-language query layer is also live — folded into the existing Chat agent (`backend/lib/orchestrator.js`) rather than a separate `POST /query/nl` endpoint, per an explicit design-fork call made when it was actually built; the concept in §5 above still describes the underlying approach accurately (LLM translates intent, never touches data directly), just not the exact endpoint shape. See `docs/part2-roadmap.md` for the full Part 2 build history, phase-by-phase, and its own Backlog section for what's genuinely still open (an iOS app is next, plus several smaller scoped items) — that document is the actively maintained one; this file's Part 2 mentions elsewhere are historical planning context, not live status. Auth details (username-based login, email verification backlog) are covered in §4.5.
